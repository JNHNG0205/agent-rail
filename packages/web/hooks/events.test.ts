...
test("an amount is read as minor units through BigInt", () => {
  ...
  assert.equal(usdcFrom({ amount: "10000000" }), "10.000000 USDC");
  assert.equal(usdcFrom({ amount: "2500000" }), "2.500000 USDC");
  assert.equal(usdcFrom({ amount: "1" }), "0.000001 USDC");
});

test("a refund is an amount too", () => {
  // JobCancelled carries `refund`, not `amount`. Missing it would show every
  // refund as zero.
  assert.equal(usdcFrom({ refund: "5000000" }), "5.000000 USDC");
});
...
test("an amount beyond a float's range is still exact", () => {
  ...
  assert.equal(usdcFrom({ amount: "9007199254740993" }), "9007199254.740993 USDC");
});
...
test("a settlement shows its amount", () => {
  ...
  assert.equal(formatted.formattedAmount, "10.000000 USDC");
  assert.match(formatted.details, /Job #42 settled and completed \(10\.000000 USDC\)/);
});
